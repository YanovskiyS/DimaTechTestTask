...
from src.schemas.users import UserRequestAdd, UserAdd, UserPatch
...
async def add_user(data: UserRequestAdd, get_id: UserIdDep, db: DbDep):
    ...
    if user.is_admin:
        ...
        raise HTTPException(
            status_code=401, detail="Вы не аторизованы как администратор"
        )
...
async def delete_user(user_id: int, get_id: UserIdDep, db: DbDep):
    ...
    if user.is_admin:
        ...
        raise HTTPException(
            status_code=401, detail="УВы не аторизованы как администратор"
        )
